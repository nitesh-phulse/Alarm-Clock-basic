...
let tone=new Audio("./alarm-sound.mp3");      // adding alarm music 
...
function currTime(){   
   ...
   //set am or pm (cycle 12hr) 
      ...
      currTime_display.innerHTML=`${hr}:${min}:${sec}:${currampm}`;
      currTime_display=`${hr}:${min}::${sec}${currampm}`;
...
// Function to remove an alarm div based on the alarmcnt parameter
function delete_alarm(alarmcnt) {
   // Get the alarm div element to be removed based on the alarmcnt
   var remove_id = document.getElementById("alarm" + alarmcnt);

   // Calculate the index in the alarmList array for the alarm to be removed
   var del = alarmcnt - 1;

   // Remove the alarm element from the alarmList array at the calculated index
   // The element is replaced with an empty string (" ") in this case, which might not be the desired behavior.
   alarmList.splice(del, 1, " ");

   // Remove the alarm div element from the DOM
   remove_id.remove();
}
